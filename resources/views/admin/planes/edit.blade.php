        ...
        <div class="max-w-xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl rounded-3xl border border-gray-100 p-8">
                ...
                <div class="mb-6 border-b pb-4">
                    ...
                    <h2 class="text-xl font-extrabold text-slate-900 mt-1">Editar Plan de Estudio</h2>
                    <p class="text-sm text-slate-500">Actualiza la configuración del plan académico.</p>
                </div>
                ...
                <form method="POST" action="{{ route('admin.planes.update', $plan->id) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <select name="programa_academico_id" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-[#0b2545] text-sm">
                            @foreach($programas as $prog)
                                <option value="{{ $prog->id }}" {{ $plan->programa_academico_id == $prog->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="tipo" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-[#0b2545] text-sm">
                            <option value="Nuevo" {{ $plan->tipo === 'Nuevo' ? 'selected' : '' }}>Nuevo</option>
                            <option value="Antiguo" {{ $plan->tipo === 'Antiguo' ? 'selected' : '' }}>Antiguo</option>
                        </select>
                    </div>

                    <div>
                        <label class="block text-xs font-extrabold uppercase tracking-wider text-slate-600 mb-1.5">Número de Semestres *</label>
                        <input type="number" name="semestres" value="{{ old('semestres', $plan->semestres) }}" required min="1" max="12"
