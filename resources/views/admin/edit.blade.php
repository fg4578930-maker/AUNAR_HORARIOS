        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-2xl border border-gray-100 p-8">
                ...
                <div class="border-b border-gray-100 pb-4 mb-6">
                    ...
                    <h3 class="text-2xl font-black text-[#0b2545] mt-2">Actualizar Datos de la Asignatura</h3>
                </div>
                ...
                <form action="{{ route('admin.asignaturas.update', $asignatura->id) }}" method="POST">
                    ...
                    <div class="mb-5">
                        ...
                        <select name="programa_id" id="programa_id" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#0b2545] focus:ring-[#0b2545] text-sm py-3" required onchange="cargarInfoPrograma(this.value)">
                            ...
                            @foreach($programas as $prog)
                                <option value="{{ $prog->id }}" {{ old('programa_id', $asignatura->programa_id) == $prog->id ? 'selected' : '' }}>{{ $prog->nombre }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="codigo" id="codigo" value="{{ old('codigo', $asignatura->codigo) }}" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#0b2545] focus:ring-[#0b2545] text-sm py-3" required>
                    </div>
                    ...
                    <div class="mb-5">
                        ...
                        <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $asignatura->nombre) }}" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#0b2545] focus:ring-[#0b2545] text-sm py-3" required>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-5 mb-5">
                        <div>
                            ...
                            <select name="plan_estudios" id="plan_estudios" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#0b2545] focus:ring-[#0b2545] text-sm py-3" required>
                                <option value="Antiguo" {{ old('plan_estudios', $asignatura->plan_estudios) == 'Antiguo' ? 'selected' : '' }}>Antiguo</option>
                                <option value="Nuevo" {{ old('plan_estudios', $asignatura->plan_estudios) == 'Nuevo' ? 'selected' : '' }}>Nuevo</option>
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <select name="semestre" id="semestre" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#0b2545] focus:ring-[#0b2545] text-sm py-3" required>
                                @for($i = 1; $i <= 9; $i++)
                                    <option value="{{ $i }}" {{ old('semestre', $asignatura->semestre) == $i ? 'selected' : '' }}>Semestre {{ $i }}</option>
                                @endfor
                            </select>
                        </div>
                        ...
                        <div>
                            ...
                            <input type="number" name="creditos" id="creditos" value="{{ old('creditos', $asignatura->creditos) }}" min="1" max="10" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#0b2545] focus:ring-[#0b2545] text-sm py-3" required>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                        <div>
                            ...
                            <select name="tipo" id="tipo" class="block w-full rounded-xl border-gray-200 shadow-sm focus:border-[#0b2545] focus:ring-[#0b2545] text-sm py-3" required>
                                <option value="Teórico" {{ old('tipo', $asignatura->tipo) == 'Teórico' ? 'selected' : '' }}>Teórico</option>
                                <option value="Teórico-Práctico" {{ old('tipo', $asignatura->tipo) == 'Teórico-Práctico' ? 'selected' : '' }}>Teórico-Práctico</option>
                                <option value="Práctico" {{ old('tipo', $asignatura->tipo) == 'Práctico' ? 'selected' : '' }}>Práctico</option>
                            </select>
                        </div>
