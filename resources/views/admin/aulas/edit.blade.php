<x-app-layout>
        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex justify-between items-center">
                <div>
                    ...
                    <h2 class="text-2xl font-extrabold text-slate-900 tracking-tight">Editar Aula o Laboratorio</h2>
                </div>
            ...
            <div class="bg-white shadow-xl rounded-2xl border border-gray-100 p-8">
                <form action="{{ route('admin.aulas.update', $aula->id) }}" method="POST" class="space-y-6">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="text" name="nombre" id="nombre" value="{{ old('nombre', $aula->nombre) }}" required placeholder="Ej. Laboratorio 3, Taller 1" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="piso" id="piso" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                ...
                                <option value="1" {{ old('piso', $aula->piso) == '1' ? 'selected' : '' }}>Piso 1</option>
                                <option value="2" {{ old('piso', $aula->piso) == '2' ? 'selected' : '' }}>Piso 2</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div>
                            ...
                            <input type="number" name="capacidad_max" id="capacidad_max" value="{{ old('capacidad_max', $aula->capacidad_max) }}" min="1" required placeholder="Ej. 30" class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                        </div>
                        ...
                        <div>
                            ...
                            <select name="tipo" id="tipo" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm">
                                ...
                                <option value="Taller de diseño" {{ old('tipo', $aula->tipo) == 'Taller de diseño' ? 'selected' : '' }}>Taller de diseño</option>
                                <option value="Sala móvil" {{ old('tipo', $aula->tipo) == 'Sala móvil' ? 'selected' : '' }}>Sala móvil</option>
                                <option value="Otro" {{ old('tipo', $aula->tipo) == 'Otro' ? 'selected' : '' }}>Otro</option>
                            </select>
                        </div>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="es_uso_general" id="es_uso_general" required class="w-full rounded-xl border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 text-sm" onchange="toggleExclusividad()">
                            <option value="1" {{ old('es_uso_general', $aula->es_uso_general) == '1' ? 'selected' : '' }}>Uso General (Disponible para todos)</option>
                            <option value="0" {{ old('es_uso_general', $aula->es_uso_general) == '0' ? 'selected' : '' }}>Exclusivo para Programas Académicos</option>
                        </select>
                    </div>
                    ...
                    <div id="seccion_programas" class="p-4 bg-slate-50 rounded-xl border border-gray-200 hidden">
                        ...
                        @php
                            $programasAsociadosIds = old('programas_ids', $aula->programasAcademicos->pluck('id')->toArray());
                        @endphp
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-2 bg-white rounded-lg border border-gray-200">
                            @foreach($programas as $programa)
                                <label class="flex items-center space-x-2 text-sm text-slate-700 p-1 hover:bg-slate-50 rounded cursor-pointer">
                                    <input type="checkbox" name="programas_ids[]" value="{{ $programa->id }}" class="programa-checkbox rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ in_array($programa->id, $programasAsociadosIds) ? 'checked' : '' }}>
                    ...
                    <div>
                        ...
                        @php
                            $asignaturasAsociadasIds = old('asignaturas_ids', $aula->asignaturas->pluck('id')->toArray());
                        @endphp
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-2 max-h-48 overflow-y-auto p-3 bg-slate-50 rounded-xl border border-gray-200">
                            @foreach($asignaturas as $asignatura)
                                <label class="flex items-center space-x-2 text-sm text-slate-700 p-1 hover:bg-white rounded cursor-pointer">
                                    <input type="checkbox" name="asignaturas_ids[]" value="{{ $asignatura->id }}" class="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500" {{ in_array($asignatura->id, $asignaturasAsociadasIds) ? 'checked' : '' }}>
    ...
    <script>
        ...
        // Ejecutar al cargar para verificar estado inicial de la base de datos o old()
