        ...
        <div class="max-w-xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl rounded-2xl border border-gray-100 p-8">
                ...
                <div class="mb-6 border-b pb-4">
                    ...
                    <h2 class="text-xl font-bold text-slate-800 mt-1">Editar Asignatura: {{ $asignatura->nombre }}</h2>
                    <p class="text-sm text-slate-500">Actualiza los datos institucionales de la materia.</p>
                </div>
                ...
                <form method="POST" action="{{ route('admin.asignaturas.update', $asignatura->id) }}" class="space-y-5">
                    @csrf
                    @method('PUT')
                    ...
                    <div>
                        ...
                        <input type="number" name="creditos" value="{{ old('creditos', $asignatura->creditos) }}" required min="1" max="10"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-[#0b2545] text-sm" />
                    </div>
                    ...
                    <div>
                        ...
                        <select name="tipo" required class="w-full px-4 py-2.5 border border-gray-300 rounded-xl shadow-sm focus:ring-2 focus:ring-[#0b2545] text-sm">
                            <option value="Teórico" {{ $asignatura->tipo === 'Teórico' ? 'selected' : '' }}>Teórico</option>
                            <option value="Práctico" {{ $asignatura->tipo === 'Práctico' ? 'selected' : '' }}>Práctico</option>
                            <option value="Teórico/Práctico" {{ $asignatura->tipo === 'Teórico/Práctico' ? 'selected' : '' }}>Teórico/Práctico</option>
                        </select>
                    </div>
